...
def _expand_variable_match(positional_vars, named_vars, match):
    ...
    if name is not None:
        try:
            ...
            raise ValueError(
                f"Named variable '{name}' not specified and needed by template `{match.string}` at position {match.start()}"
            )
        ...
        try:
            ...
            raise ValueError(
                f"Positional variable not specified and needed by template `{match.string}` at position {match.start()}"
            )
    else:
...
def validate(tmpl, path):
    ...
    """
    pattern = f"{_generate_pattern_for_template(tmpl)}$"
