...
def from_dict(data, require=None):
    ...
    if missing := keys_needed.difference(six.iterkeys(data)):
        raise ValueError(
            f"Service account info was not in the expected format, missing fields {', '.join(missing)}."
        )

    return crypt.RSASigner.from_service_account_info(data)
