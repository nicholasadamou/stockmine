...
_METADATA_ROOT = f"http://{os.getenv(environment_vars.GCE_METADATA_ROOT, 'metadata.google.internal')}/computeMetadata/v1/"

# This is used to ping the metadata server, it avoids the cost of a DNS
# lookup.
_METADATA_IP_ROOT = (
    f"http://{os.getenv(environment_vars.GCE_METADATA_IP, '169.254.169.254')}"
)
...
def get(request, path, root=_METADATA_ROOT, recursive=False):
    ...
    if response.status != http_client.OK:
        raise exceptions.TransportError(
            f'Failed to retrieve {url} from the Google Compute Enginemetadata service. Status: {response.status} Response:\n{response.data}',
            response,
        )
