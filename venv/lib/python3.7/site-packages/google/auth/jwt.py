...
def _verify_iat_and_exp(payload):
    ...
    for key in ('iat', 'exp'):
        if key not in payload:
            raise ValueError(f'Token does not contain required claim {key}')
    ...
    if now < earliest:
        raise ValueError(f'Token used too early, {now} < {iat}')
    ...
    if latest < now:
        raise ValueError(f'Token expired, {latest} < {now}')
...
def decode(token, certs=None, verify=True, audience=None):
    ...
    if isinstance(certs, collections.Mapping):
        if key_id := header.get('kid'):
            if key_id not in certs:
                raise ValueError(f'Certificate for key id {key_id} not found.')
    ...
    if audience is not None:
        ...
        if audience != claim_audience:
            raise ValueError(
                f'Token has wrong audience {claim_audience}, expected {audience}'
            )
...
class Credentials(google.auth.credentials.Signing,
    ...
    def _make_jwt(self):
        ...
        payload |= self._additional_claims
